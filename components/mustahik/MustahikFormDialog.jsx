...
export default function MustahikFormDialog({ open, onClose, mustahik = null, wilayahOptions = [] }) {
            ...
            setForm(
                mustahik
                    ? {
                        ...
                        wilayahName: mustahik.wilayah?.name ?? "",
                    }
                    : BLANK
            );
            setError("");
    ...
    // component controlled by `open` prop
    return (
        <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
            <DialogContent className="w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{isEdit ? "Edit Mustahik" : "Tambah Mustahik Baru"}</DialogTitle>
                </DialogHeader>

                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <ZakatFormField label="Nama Lengkap" required className="col-span-1 sm:col-span-2">
                            <input
                                type="text"
                                className={inputCls}
                        ...
                        <ZakatFormField label="Wilayah (opsional)">
                            <input
                                type="text"
                                list="wilayah-datalist"
                                className={inputCls}
                                placeholder="misal: karanganyar"
                                value={form.wilayahName}
                                onChange={set("wilayahName")}
                            />
                            ...
                            <p className="text-xs text-gray-400 mt-1">
                                Ketik nama baru atau pilih dari daftar. Disimpan dalam huruf kecil.
                            </p>
                        </ZakatFormField>

                        <ZakatFormField label="Alamat" className="col-span-1 sm:col-span-2">
                            <textarea
