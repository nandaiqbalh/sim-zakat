...
export async function createMustahik({ mosqueId, name, address, phone, category, isActive = true }) {
  try {
...
export async function findMustahikById(id) {
  try {
    const mustahik = await prisma.mustahik.findUnique({ where: { id } });
...
export async function updateMustahik(id, data) {
  try {
    const mustahik = await prisma.mustahik.update({ where: { id }, data });
